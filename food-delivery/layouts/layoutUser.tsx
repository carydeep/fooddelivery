...
function LayoutUser({ children }: { children: ReactNode }) {
  ...
  const userInfo = useAppSelector(
    (state) => state.userInfo.current.user_metadata
  );
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.function}>
        <img
          className={styles.function__image}
          src={user?.picture || undefined}
        />
        <h3 className={styles.function__name}>
          {userInfo?.name || user?.name}
        </h3>
        <ul className={styles.function__option}>
          <li
            className={`${styles.function__option__item} ${
              router.asPath === "/user" && styles.active
            }`}
          >
            <Link href={`/user`}>
              <a>
                <AiOutlineHome
                  className={styles.function__option__item__icon}
                />
                Home
              </a>
            </Link>
          ...
          <li
            className={`${styles.function__option__item} ${
              router.asPath === "/user/catalog" && styles.active
            }`}
          >
            <Link href={`/user/catalog`}>
              <a>
                <BsInboxes className={styles.function__option__item__icon} />
                Catalog
              </a>
            </Link>
          </li>
          ...
          <li className={styles.function__option__item}>
            <BiMap className={styles.function__option__item__icon} />
            Map
          </li>
          <li className={styles.function__option__item}>
            <MdDeliveryDining className={styles.function__option__item__icon} />
            Delivery
          </li>
          <li className={styles.function__option__item}>
            <GoSettings className={styles.function__option__item__icon} />
            Setting
          </li>
        </ul>
        <img
          src="../advertise-app.png"
          className={styles.function__advertise}
        ></img>
        <div className={styles.function__download}>
          <BiDownload className={styles.function__download__icon} />
          Mobile App
        </div>
      </div>
        ...
        <div className={styles.header}>
          <form className={styles.header__form}>
            ...
            <datalist id="listFood">
              {foods &&
                foods.map((food) => {
                  return <option key={food.id} value={food.name}></option>;
                })}
            </datalist>
            <button className={styles.header__form__button}>
              <BsSearch />
            </button>
          </form>
          <ul className={styles.header__notice}>
            <li style={{ position: "relative" }}>
              <IoMdNotificationsOutline
                className={styles.header__notice__item}
              />
            </li>
            <li
            ...
            >
              ...
              {orders?.orderItems && (
                <span className={styles.header__notice__note}>
            ...
            <li style={{ position: "relative" }}>
              <Link href="/api/auth/logout">
                <a>
                  <IoLogOutOutline
                    className={`${styles.header__notice__item} ${styles.warning}`}
                  />
                </a>
              </Link>
            </li>
